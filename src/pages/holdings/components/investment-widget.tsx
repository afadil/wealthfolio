...
function getCurrencyData(holdings: Holding[] = [], baseCurrency: string): CurrencyData[] {
  ...
  const currencies = holdings.reduce<Record<string, number>>((acc, holding) => {
    ...
    const marketValue = Number(holding.marketValueConverted) || 0;
        ...
        <div className="space-y-6">
          ...
          <div className="mt-2">
            {data.map((currency, index) => (
              <div
                key={currency.name}
                className="flex items-center justify-between gap-4 py-1 rounded-md transition-colors hover:bg-muted/50"
              >
