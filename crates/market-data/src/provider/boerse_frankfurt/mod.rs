...
impl BoerseFrankfurtProvider {
    ...
    fn build_headers(timestamp: &str, full_url: &str, salt: &str) -> reqwest::header::HeaderMap {
        ...
        let minute_str = timestamp
            ...
            .collect::<String>();
        let security = format!("{:x}", md5::compute(minute_str.as_bytes()));
        ...
        let trace_id = format!("{:x}", md5::compute(trace_input.as_bytes()));
...
mod tests {
    ...
    fn test_build_headers() {
        ...
        let expected_security = format!("{:x}", md5::compute("202602160609".as_bytes()));
        assert_eq!(
