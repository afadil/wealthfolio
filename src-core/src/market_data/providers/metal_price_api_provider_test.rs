...
mod tests {
    ...
    use crate::market_data::{AssetProfiler, market_data_errors::MarketDataError};
    ...
    async fn test_get_asset_profile_gold() {
        ...
        assert_eq!(profile.categories, Some("Precious Metals,Physical Commodities".to_string()));
        assert_eq!(profile.classes, Some("Physical Commodity".to_string()));
        assert_eq!(profile.attributes, Some("Safe Haven,Inflation Hedge,Store of Value".to_string()));
        assert_eq!(profile.sectors, Some("Materials,Commodities".to_string()));
        assert!(profile.notes.is_some());
        assert!(profile.notes.as_ref().unwrap().contains("Gold is a precious metal"));
    ...
    async fn test_get_asset_profile_silver() {
        ...
        assert!(profile.categories.as_ref().unwrap().contains("Industrial Metals"));
        assert!(profile.attributes.as_ref().unwrap().contains("Industrial Use"));
    }
    ...
    async fn test_get_asset_profile_platinum() {
        ...
        assert!(profile.attributes.as_ref().unwrap().contains("Automotive Industry"));
    }
    ...
    async fn test_get_asset_profile_palladium() {
        ...
        assert!(profile.notes.as_ref().unwrap().contains("catalytic converters"));
    }
    ...
    async fn test_all_supported_metals_have_complete_profiles() {
        ...
        for metal in metals {
            ...
            assert!(profile.asset_type.is_some(), "Missing asset_type for {}", metal);
            assert!(profile.asset_class.is_some(), "Missing asset_class for {}", metal);
            assert!(profile.asset_sub_class.is_some(), "Missing asset_sub_class for {}", metal);
            assert!(profile.notes.is_some(), "Missing notes for {}", metal);
            assert!(profile.countries.is_some(), "Missing countries for {}", metal);
            assert!(profile.categories.is_some(), "Missing categories for {}", metal);
            assert!(profile.classes.is_some(), "Missing classes for {}", metal);
            assert!(profile.attributes.is_some(), "Missing attributes for {}", metal);
            assert!(profile.sectors.is_some(), "Missing sectors for {}", metal);
            assert!(profile.url.is_some(), "Missing url for {}", metal);
            assert!(!profile.currency.is_empty(), "Missing currency for {}", metal);
            assert!(!profile.data_source.is_empty(), "Missing data_source for {}", metal);
