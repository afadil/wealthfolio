...
pub trait SnapshotServiceTrait: Send + Sync {
    ...
    /// Either way the scope is expanded with the source account of every
    /// paired `TRANSFER_IN` in scope, so transferred lots keep their original
    /// cost basis (see [`SnapshotService::expand_scope_to_transfer_sources`]).
...
impl SnapshotService {
    ...
    async fn calculate_holdings_snapshots_internal(
        ...
        // Resolve the requested scope (every active TRANSACTIONS account when
        // `None`) and expand it with the source account of each paired
        // TRANSFER_IN in scope, so the paired TRANSFER_OUT is replayed in the
        // same run and the destination keeps the transferred cost basis.
        let requested_account_ids: Vec<String> = match account_ids_param {
        ...
        let scoped_account_ids = self.expand_scope_to_transfer_sources(&requested_account_ids)?;

        // Enter the shared gate before reading the activities that feed the
        // calculation. Acquiring after `fetch_required_data` would serialize
        // the writes while still allowing a queued recalculation to run from
        // data fetched before the prior one completed. The scope expansion
        // above only reads activities to decide *which* accounts to lock; the
        // data actually replayed is fetched below, under the gate.
        let recalculation_permit: Option<PortfolioRecalculationPermit> =
            match &self.recalculation_gate {
                Some(gate) => Some(gate.acquire(&scoped_account_ids).await),
                None => None,
            };

        let (accounts_to_process, all_activities, min_activity_date, calculation_end_date) =
            self.fetch_required_data(&scoped_account_ids)?;
    ...
    /// Expands a recalculation scope so that every paired `TRANSFER_IN` in scope
    /// has the account holding its `TRANSFER_OUT` in scope as well.
    ///
    /// Transferred lots (original acquisition dates and cost basis) travel from
    /// the `TRANSFER_OUT` to the `TRANSFER_IN` only through the in-memory
    /// `ProjectionRun` transfer cache. If the source account is not part of the
    /// same run its leg is never replayed, the cache stays empty, and the
    /// `TRANSFER_IN` handler silently falls back to the transfer's own unit
    /// price. Pulling the source account in keeps the basis intact; the
    /// existing per-day topological sort then orders the two legs correctly.
    ///
    ...
    fn expand_scope_to_transfer_sources(&self, requested: &[String]) -> Result<Vec<String>> {
        let mut scope: Vec<String> = Vec::with_capacity(requested.len());
        let mut in_scope: HashSet<String> = HashSet::with_capacity(requested.len());
        for account_id in requested {
            if in_scope.insert(account_id.clone()) {
                scope.push(account_id.clone());
            }
        }

        let mut frontier = scope.clone();
        while !frontier.is_empty() {
            let activities = self
                .activity_repository
                .get_activities_by_account_ids(&frontier)?;

            // Paired TRANSFER_IN legs in this batch, keyed by group id, minus
            // the groups whose TRANSFER_OUT is already in the batch (their
            // source is in scope by construction, no lookup needed).
            let mut transfer_in_by_group: HashMap<&str, &Activity> = HashMap::new();
            let mut groups_with_out_in_batch: HashSet<&str> = HashSet::new();
            ...
            let mut lookups: Vec<(&str, &Activity)> = transfer_in_by_group
                .into_iter()
                .filter(|(group_id, _)| !groups_with_out_in_batch.contains(group_id))
                .collect();
            lookups.sort_by(|a, b| a.0.cmp(b.0));
            for (group_id, transfer_in) in lookups {
                let Some(counterpart) = self
                    .activity_repository
                    .find_transfer_counterpart(group_id, &transfer_in.id)?
                else {
                    continue;
                };
                let source_account_id = counterpart.account_id;
                if in_scope.contains(&source_account_id) {
                    continue;
                }
                match self.account_repository.get_by_id(&source_account_id) {
                    ...
                    Ok(account) if account.tracking_mode == TrackingMode::Holdings => {
                        debug!(
                            "Not expanding recalculation scope with account {} (source of transfer \
                             group {}): HOLDINGS tracking mode",
                            source_account_id, group_id
                        );
                    }
                    Ok(_) => {
                        info!(
                            "Expanding recalculation scope with account {}: it holds the TRANSFER_OUT \
                             of group {} whose TRANSFER_IN {} is in account {}",
                            source_account_id, group_id, transfer_in.id, transfer_in.account_id
                        );
                        in_scope.insert(source_account_id.clone());
                        added.push(source_account_id);
                    }
                    Err(err) => {
                        warn!(
                            "Cannot expand recalculation scope with account {} (source of transfer \
                             group {}): {}",
                            source_account_id, group_id, err
                        );
                    }
                }
            }
    ...
    // Fetches the accounts in `account_ids` (already expanded by
    // `expand_scope_to_transfer_sources`) and activities ONLY for those accounts.
    fn fetch_required_data(
        ...
        // Process the ids regardless of is_active status (per spec: "process
        // exactly ids even inactive/archived for per-account rebuild").
