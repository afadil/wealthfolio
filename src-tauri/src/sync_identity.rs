...
pub fn get_or_create_sync_identity() -> Result<(Uuid, Arc<transport::Identity>)> {
    ...
    let legacy_device_id = SecretManager::get_secret(LEGACY_DEVICE_ID_SECRET_KEY)?
    ...
    if legacy_device_id.is_some() {
        let _ = SecretManager::delete_secret(LEGACY_DEVICE_ID_SECRET_KEY);
    }
...
fn persist_identity(device_id: &Uuid, identity: &transport::Identity) -> Result<()> {
    ...
    SecretManager::set_secret(SYNC_IDENTITY_SECRET_KEY, &serialized)?;
    Ok(())
}
