...
async function createPairingApi(
  ...
  return invokeTauri("create_pairing", { codeHash, ephemeralPublicKey });
}
...
async function claimPairingApi(
  ...
  return invokeTauri("claim_pairing", { code, ephemeralPublicKey });
}
...
class SyncService {
  ...
  private async doInitialize(): Promise<string> {
    ...
    if (existingDeviceId) {
      ...
      await invokeTauri("set_device_id", { deviceId: this.deviceId }).catch(() => {
  ...
  /**
   * Enable E2EE for the team (owner only, first device)
   */
  async enableE2EE(): Promise<{ keyVersion: number }> {
    ...
    try {
      // Enable on server (server auto-trusts this device)
      const result = await enableE2eeApi();

      if (!result.e2eeEnabled) {
        throw new SyncError("E2EE_ENABLE_FAILED", "Failed to enable E2EE");
      }

      // Generate and store Root Key locally
  ...
  async checkTrustStatus(): Promise<{
    ...
    reason?: "no_key" | "version_mismatch" | "not_trusted";
  }> {
    ...
    if (device.trustState !== "trusted") {
      return { needsPairing: true, reason: "not_trusted" };
    }
  ...
  async sendRootKey(session: PairingSession): Promise<void> {
    ...
    // Get claimer device ID from session (poll to find it)
    const devices = await this.getDevices();
    const claimerDevice = devices.find((d) => !d.isCurrent && d.trustState === "untrusted");

    if (!claimerDevice) {
      throw new SyncError("CLAIMER_NOT_FOUND", "Claimer device not found");
    }

    // Send to server (base64 encode the JSON payload)
  ...
  async pollForRootKey(claim: ClaimResult): Promise<boolean> {
    ...
    if (result.sessionStatus === "canceled" || result.sessionStatus === "expired") {
