 ...
 */
export function useSwingDashboard(period: PeriodType) {
  const { data: activities } = useSwingActivities();
  const { preferences } = useSwingPreferences();
  const { exchangeRates, baseCurrency } = useCurrencyConversion({ enabled: true });
  ...
  return useQuery({
    ...
    enabled: !!activities && !!exchangeRates,
...
function updateOpenPositionsWithMarketPrices(
  ...
  return openPositions.map((position) => {
    ...
    if (matchingHolding?.price != null && matchingHolding.price > 0) {
      // Get current price and ensure it's in the same currency as position
...
function filterTradesByPeriod(
  ...
  // Filter trades by exit date within period
