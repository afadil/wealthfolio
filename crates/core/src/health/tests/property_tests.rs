//! Property-based tests for the Health Center.
//!
...
trait WithMvPct {
    fn with_mv_pct(self, mv_pct: Option<f64>) -> Self;
}

impl WithMvPct for HealthIssue {
    fn with_mv_pct(mut self, mv_pct: Option<f64>) -> Self {
        self.affected_mv_pct = mv_pct;
        self
    }
}
...
proptest! {
    ...
    /// (This tests our test data generation, not the actual implementation)
    #[test]
    fn prop_unique_issue_ids_generation(
        issues in arb_health_issues(50)
    ) {
        // Note: Our generator produces unique IDs because each has a unique hash
        let ids: HashSet<_> = issues.iter().map(|i| &i.id).collect();

        // We can't guarantee uniqueness in generated data, but we verify
        // that if we had duplicates, they'd be detectable
...
// =============================================================================
// Additional Unit Tests (non-property-based but useful)
// =============================================================================
...
mod unit_tests {
    ...
    #[test]
    fn test_severity_max_with_critical_present() {
        let issues = vec![
            HealthIssue::builder()
                .id("test1")
        ...
        assert_eq!(status.overall_severity, Severity::Critical);
    }

    #[test]
    fn test_all_severities_counted() {
        let issues = vec![
            HealthIssue::builder()
                .id("info1")
            ...
            HealthIssue::builder()
                ...
                .title("Test")
                .message("Test message")
                .data_hash("hash_error")
                .build(),
        ];

        let status = HealthStatus::from_issues(issues);

        assert_eq!(status.issue_counts.get(&Severity::Info), Some(&1));
