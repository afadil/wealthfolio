  ...
  if (!importSummary) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-muted-foreground mt-4">{t("import_validating")}</p>
      </div>
    );
  }
    ...
    >
      {hasImportErrors ? (
        <div className="space-y-4">
          <ImportAlert
            variant="destructive"
            title={t("import_all_failed")}
            description={t("import_error_occurred")}
          />
          ...
          <Card className="bg-success/5 border-success/20 mx-auto w-full max-w-lg">
            <CardContent className="flex flex-col items-center space-y-5 pt-8 pb-10 text-center">
              ...
              <motion.div className="space-y-2" variants={textVariants}>
                <h2 className="text-success/90 text-xl font-semibold">{t("import_successful")}</h2>
                <p className="text-muted-foreground text-sm">
                  {t("import_successfully_imported", { count: totalRows })}
                </p>
              </motion.div>
            </CardContent>
          </Card>
