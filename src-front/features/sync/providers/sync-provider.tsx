...
type SyncAction =
  ...
  | { type: "PAIRING_CLAIMED"; claimerPublicKey: string; sessionKey: string }
...
export function SyncProvider({ children }: { children: ReactNode }) {
        ...
        try {
          ...
          try {
            ...
            dispatch({
              ...
              trustState: needsPairing ? "untrusted" : (device.trustState as TrustState),
  ...
  const enableE2EE = useCallback(async () => {
    const { keyVersion } = await syncService.enableE2EE();
    const status = await syncService.getSyncStatus();
    dispatch({ type: "SET_SYNC_STATUS", status });
