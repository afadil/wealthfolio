...
export class PerformanceCalculator {
  ...
  calculateRealizedPLForPeriod(
    ...
    for (const trade of this.closedTrades) {
      if (isWithinInterval(trade.exitDate, { 
  ...
  calculateDistribution(fxRateMap: Record<string, number> = {}): TradeDistribution {
    ...
    for (const trade of this.closedTrades) {
      ...
      this.updateDistributionRecord(byHoldingPeriod, holdingBucket, convertedPL, trade.returnPercent)
  ...
  private calculateAverageHoldingDays(): number {
    ...
    const totalDays = this.closedTrades.reduce(
      (sum, trade) => sum + trade.holdingPeriodDays, 
      0
    )
