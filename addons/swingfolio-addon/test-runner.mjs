...
async function runTest() {
  ...
  if (!fs.existsSync(csvPath)) {
    ...
    console.error(
      "Please ensure the activities_2025-08-25.csv file is in the addon's root folder.",
    );
    return;
  }
  ...
  const results = {};
  const fxRates = { USD: 1.38555, CAD: 1.0 }; // Base currency is CAD, use actual app rate
  ...
  methods.forEach((method) => {
    ...
    feeOptions.forEach((includeFees) => {
      ...
      const mayPL = performanceCalculator.calculateRealizedPLForPeriod(startDate, endDate, fxRates);
