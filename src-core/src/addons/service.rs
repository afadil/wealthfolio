...
// Constants
// Note: Addon store is currently disabled. Update this URL when you have your own addon store API.
...
pub fn parse_manifest_json_metadata(manifest_content: &str) -> Result<AddonManifest, String> {
    ...
    let min_wealthfolio_version = raw_manifest["minWealthVNVersion"]
        .as_str()
        .map(|s| s.to_string());
