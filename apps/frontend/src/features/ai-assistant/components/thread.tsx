...
const ThreadScrollToBottom: FC = () => {
  ...
  return (
    <ThreadPrimitive.ScrollToBottom behavior="smooth" asChild>
      <TooltipIconButton
        tooltip={t("ai.thread.scroll_to_bottom")}
        variant="outline"
      ...
      {[
        {
          ...
          text: t("ai.thread.suggestion.performance_year"),
        },
        {
          icon: "BarChart" as const,
          text: t("ai.thread.suggestion.top_holdings"),
        },
        {
          icon: "FileText" as const,
          text: t("ai.thread.suggestion.dividend_summary"),
        },
        {
          icon: "PieChart" as const,
          text: t("ai.thread.suggestion.asset_allocation"),
        },
    ...
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone className="aui-composer-attachment-dropzone border-input bg-background has-[textarea:focus-visible]:border-ring has-[textarea:focus-visible]:ring-ring/50 data-[dragging=true]:border-ring data-[dragging=true]:bg-accent/50 dark:bg-background shadow-xs flex w-full flex-col rounded-3xl border px-1 pt-2 outline-none transition-[color,box-shadow] has-[textarea:focus-visible]:ring-[3px] data-[dragging=true]:border-dashed">
        ...
        <ComposerPrimitive.Input
          placeholder={t("ai.thread.composer_placeholder")}
          ...
          rows={1}
          autoFocus
          aria-label={t("ai.composer.input_aria")}
        />
    ...
    <div className="aui-composer-action-wrapper relative mx-1 mb-2 mt-2 flex items-center justify-between">
      ...
      <ThreadPrimitive.If running={false}>
        <ComposerPrimitive.Send asChild>
          <TooltipIconButton
            tooltip={t("ai.composer.send_tooltip")}
            side="bottom"
            type="submit"
            variant="default"
            size="icon"
            className="aui-composer-send size-[34px] rounded-full p-1"
            aria-label={t("ai.composer.send_aria")}
          >
      ...
      <ThreadPrimitive.If running>
        <ComposerPrimitive.Cancel asChild>
          <Button
            ...
            aria-label={t("ai.composer.stop_aria")}
          >
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Copy asChild>
        <TooltipIconButton tooltip={t("ai.message.copy_tooltip")}>
      ...
      <ActionBarPrimitive.Reload asChild>
        <TooltipIconButton tooltip={t("ai.message.refresh_tooltip")}>
          <Icons.RefreshCw />
        </TooltipIconButton>
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Edit asChild>
        <TooltipIconButton tooltip={t("ai.message.edit_tooltip")} className="aui-user-action-edit p-4">
          <Icons.Pencil />
        </TooltipIconButton>
    ...
    <MessagePrimitive.Root className="aui-edit-composer-wrapper max-w-(--thread-max-width) mx-auto flex w-full flex-col gap-4 px-2">
      <ComposerPrimitive.Root className="aui-edit-composer-root bg-muted max-w-7/8 ml-auto flex w-full flex-col rounded-xl">
        ...
        <div className="aui-edit-composer-footer mx-3 mb-3 flex items-center justify-center gap-2 self-end">
          <ComposerPrimitive.Cancel asChild>
            <Button variant="ghost" size="sm" aria-label={t("ai.composer.cancel_edit_aria")}>
              {t("ai.composer.cancel_edit")}
            </Button>
          </ComposerPrimitive.Cancel>
          <ComposerPrimitive.Send asChild>
            <Button size="sm" aria-label={t("ai.composer.update_message_aria")}>
              {t("ai.composer.update_message")}
            </Button>
          </ComposerPrimitive.Send>
        </div>
      </ComposerPrimitive.Root>
  ...
  return (
    ...
    >
      <BranchPickerPrimitive.Previous asChild>
        <TooltipIconButton tooltip={t("settings.shared.previous")}>
          <Icons.ChevronLeft />
        </TooltipIconButton>
      ...
      <BranchPickerPrimitive.Next asChild>
        <TooltipIconButton tooltip={t("settings.shared.next")}>
          <Icons.ChevronRight />
        </TooltipIconButton>
