...
> Complete specification for provider-agnostic asset management and
...
1. \***\*Asset Model\*\*** - Single `assets` table supporting all portfolio
   asset kinds
2. \***\*Pricing Modes\*\*** - Manual pricing as first-class alongside market
   pricing
3. \***\*Provider-Agnostic Resolution\*\*** - No provider symbols stored on
   assets; resolution via chain
4. \***\*Market Data Crate\*\*** - Standalone `wealthfolio-market-data` crate
   with multi-provider orchestration

**## Scope**

\***\*In Scope:\*\***

- All portfolio asset kinds (security, crypto, FX, cash, property, liability,
  etc.)
...
1. \***\*Manual quotes\*\*** (`source='MANUAL'`) are \***\*never
   overwritten\*\*** by provider refresh
2. \***\*Manual quote entry\*\***: Daily granularity, supports CSV import for
   bulk entry
...
4. If `pricing_mode = MARKET`:
   ...
   - \***\*Auto-backfill\*\***: Historical quotes fetched automatically on asset
     creation

**#### FX Conversion**

FX rate inversion and multi-hop conversion is handled by the existing
`CurrencyConverter` (graph-based BFS). The market data layer only fetches
canonical pairs (e.g., `EUR/USD`); inverse rates and cross rates are computed
on-the-fly.
...
3. \***\*If cannot resolve\*\*** → return `ResolutionFailed`, registry tries
   next provider
...
/// Extract canonical ticker from provider symbol
pub fn strip_yahoo_suffix(symbol: &str) -> &str {
    // Find last '.' and check if suffix looks like exchange code
    if let Some(dot_pos) = symbol.rfind('.') {
        let suffix = &symbol[dot_pos + 1..];
        // Known suffix patterns (2-4 chars, uppercase)
        if suffix.len() >= 1 && suffix.len() <= 4 && suffix.chars().all(|c| c.is_ascii_uppercase()) {
            return &symbol[..dot_pos];
        }
    }
    // Handle special suffixes
    if symbol.ends_with("=X") {  // FX
        return &symbol[..symbol.len() - 2];
    }
    if symbol.ends_with("=F") {  // Futures
        return &symbol[..symbol.len() - 2];
    }
    symbol
}
...
impl QuoteRepository {
    pub async fn upsert_quotes(
        ...
        for quote in quotes {
            // Manual quotes are never overwritten
            if self.is_manual_quote_exists(asset_id, &quote.timestamp).await? {
                continue;
            }

            // Upsert by (asset_id, timestamp, source)
            self.upsert_quote(asset_id, &quote, source).await?;
        }
        Ok(())
    }

    async fn is_manual_quote_exists(&self, asset_id: &str, ts: &DateTime<Utc>) -> Result<bool, Error> {
        // Check if manual quote exists for this date
        let sql = "SELECT 1 FROM quotes WHERE asset_id = ? AND ts = ? AND source = 'MANUAL' LIMIT 1";
        // ...
    }
}
...
4. User selects one
5. System creates asset with \***\*override only for search provider\*\***
   (rules handle other providers):
...
2. \***\*Add to Asset struct:\*\*** `exchange_mic`, `pricing_mode`,
   `preferred_provider`, `provider_overrides`, `profile_json`
...
- \***\*Germany:\*\*** XETRA, Frankfurt, Stuttgart, Hamburg, Dusseldorf, Munich,
  Berlin
...
- \***\*Central/Eastern Europe:\*\*** Swiss, Vienna, Warsaw, Prague, Budapest,
  Istanbul
