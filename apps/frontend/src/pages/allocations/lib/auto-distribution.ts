import type { Holding, TargetAllocation } from "@/lib/types";
 ...
 * @param targetAllocations - Existing holding targets
...
export function calculateAutoDistribution(
  ...
  holdings.forEach((holding) => {
    ...
    const existingTarget = targetAllocations.find((t) => t.categoryId === assetId);
    ...
    holdingMap.set(assetId, {
      ...
      currentPercent,
      targetPercent: pendingValue ?? existingTarget?.targetPercent / 100, // Convert from basis points
      isLocked,
      isUserSet,
    });
  });
