...
use super::rebalancing_model::{
    AssetClassTarget, AssetClassTargetDB, HoldingTarget, HoldingTargetDB, NewAssetClassTarget,
    NewHoldingTarget, NewRebalancingStrategy, RebalancingStrategy, RebalancingStrategyDB,
};
...
impl RebalancingRepository for RebalancingRepositoryImpl {
    async fn get_strategies(&self) -> Result<Vec<RebalancingStrategy>> {
        ...
        let strategies = rebalancing_strategies::table.load::<RebalancingStrategyDB>(&mut conn)?;
    ...
    async fn delete_asset_class_target(&self, id: &str) -> Result<()> {
        ...
        self.writer
            .exec(move |conn| {
                diesel::delete(asset_class_targets::table.find(&id)).execute(conn)?;
                Ok(())
            })
            .await
    }
    ...
    async fn delete_holding_target(&self, id: &str) -> Result<()> {
        ...
        self.writer
            .exec(move |conn| {
                diesel::delete(holding_targets::table.find(&id)).execute(conn)?;
                Ok(())
            })
            .await
    }
    ...
    async fn get_active_strategy_for_account(
        ...
        let strategy = rebalancing_strategies::table
            .filter(rebalancing_strategies::account_id.eq(account_id))
            .filter(rebalancing_strategies::is_active.eq(1))
            .first::<RebalancingStrategyDB>(&mut conn)
            .optional()?;
